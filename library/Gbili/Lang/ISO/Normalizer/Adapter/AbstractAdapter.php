...
abstract class AbstractAdapter
{
	...
	{
		$dirtyLangStr = Encoding::utf8Encode((string) $dirtyLangStr);
		do {
		...
		if (isset($langISOMatcher[$langISO]['id'])) {
			$this->langISOId = $langISOMatcher[$langISO]['id'];
		}
	...
	{
		if (null === $this->langISOId) {
			throw new Exception('The normalizer adapter does not provide any id for lang iso, call hasLangISOId() to avoid exception');
		}
		return $this->langISOId;
	}
}