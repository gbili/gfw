...
extends \Gbili\Savable\Savable
{
	...
	//originalEntity Video_Entity_Savable
	//dubbedEntity array of Video_Entity_Savable
	//sharedInfo
	...
	/**
	 * Ensure all entities get the same_ shared info pointer
	 * 
	 * @param Video_Entity_Savable $entity
	 * @return unknown_type
	 */
	private function negotiateSharedInfo(Entity\Savable $entity)
	{
		...
		if (true === $this->hasSharedInfo()) {
			...
			if ($entity->hasSharedInfo()) {
				//set this shared inf
				$this->setElement('sharedInfo', $entity->getSharedInfo());
			}
		}
	}
	 ...
	 * @return unknown_type
	 */
	public function setOriginalEntity(Entity\Savable $entity)
	{
		$this->negotiateSharedInfo($entity);
		if ($this->isSetKey('originalEntity')) {
			throw new Exception('The original entity can only be set once');
		}
		$this->setElement('originalEntity');
	}

	/**
	 * Return the original entity
	 * 
	 * @return Video_Entity_Savable
	 */
	 ...
	 * @return unknown_type
	 */
	public function addDubbedEntity(Entity\Savable $entity)
	{
		$this->negotiateSharedInfo($entity);
		$this->useKeyAsArrayAndPushValue('dubbedEntities', $entity, true, true);
	}
	...
	/**
	 * Get the value sotred in $buckets
