...
class PageSelector
{
	 ...
	 * @var unknown_type
	 */
	private static $itemsPerPagePN = 'ipp';
	
	/**
	 * 
	 * @var unknown_type
	 */
	private static $isDispalyIPP = true;
	 ...
	 * @return unknown_type
	 */
	public static function setRequestUrlPrependSegment($segment)
	{
		self::$rUPS = $segment;
	}
	 ...
	 * @return unknown_type
	 */
	public static function setPPName($name)
	{
		self::$pagePN = (string) $name;
	}
	 ...
	 * @return unknown_type
	 */
	public static function getPPName()
	{
		return self::$pagePN;
	}
	 ...
	 * @return unknown_type
	 */
	public static function setIPPName($name)
	{
		self::$itemsPerPagePN = (string) $name;
	}
	 ...
	 * @return unknown_type
	 */
	public static function getIPPName()
	{
		return self::$itemsPerPagePN;
	}
	 ...
	 * @return unknown_type
	 */
	public static function displayIPP($bool)
	{
		self::$isDispalyIPP = (boolean) $bool;
	}
	...
	{	
		...
		$this->requestUrl = (string) ((null !== self::$rUPS)? self::$rUPS : '') . $requestUrl;
		// /ipp/123 or empty string
		$this->iPPHtml = (true !== self::$isDispalyIPP)? '' : self::$itemsPerPagePN . '/' . $iPP;
		...
		if ($currentPageNumber > 1) {//if the user is viewing a page bigger than the first show <<.. link
			$html .= $this->renderHtmlPart('other_page', 1, '<<...');
			$html .= $this->renderHtmlPart('other_page', ($currentPageNumber - 1), '<');
		}
		...
		for ($i=$forStart; $i<= $forCount; $i++) { 
			$html .= $this->renderHtmlPart(((integer) $i === (integer) $currentPageNumber)? 'curr_page': 'other_page', $i, $i);
		}
		...
		if ($currentPageNumber < $totalAmountOfPages) {
			$html .= $this->renderHtmlPart('other_page', ($currentPageNumber + 1), '>');
			$html .= $this->renderHtmlPart('other_page', $totalAmountOfPages, '...>>');
		}
		return $html;
	}
	 ...
	 * @return unknown_type
	 */
	private function renderHtmlPart($class, $page, $value)
	{
		return '<div class="' . $class . '"><a href="' . $this->requestUrl . '/' . self::$pagePN . "/$page/$this->iPPHtml\">$value</a></div>";
	}
