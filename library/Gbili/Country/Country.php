...
class Country
{
	...
	public function __construct($countryStr, array $spokenLang = array())
	{
		...
		$this->isNormalized = is_array($cInfo);
		//this will propulate the country
		//with meaningfull data or with 
		//the parameters data
		if ($this->isNormalized) {
			if (!isset($cInfo['name'])) {
				throw new Exception('Normalization did not return standard array');
			}
			$countryStr = $cInfo['name'];//sanitized country str
			if (!isset($cInfo['langISO']) || !is_array($cInfo['langISO'])) {
				throw new Exception('Not standard normalization, langs must be set as an array of strings given : ' . print_r($spokenLang, true));
			}
			$spokenLang = $cInfo['langISO'];
			foreach ($spokenLang as $lang) {
				$this->langs[] = new \Gbili\Lang\Savable($lang);
			}
		}
		if ($normalizer->hasCountryId()) {
			$this->id = $normalizer->getCountryId();
		}
