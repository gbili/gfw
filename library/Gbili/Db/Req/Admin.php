...
{	
	 ...
	 * @return unknown_type
	 */
	public static function genStr($char, $len)
	{
		$str = '';
		for ($i=0; $i < $len; $i++) {
			$str .= $char;
	 ...
	 * @return unknown_type
	 */
	public static function fillEmptySpace($len, $string, $char = ' ')
	{
		$strLen = mb_strlen($string);
		if ($strLen + 2 > $len) {
			throw new Exception('the string length must be greater than the len param');
		}
	 ...
	 * @return unknown_type
	 */
	public static function genLine($separator, array $items, array $itemsMaxLenCount, $fillEmtpySpaceWith = '&nbsp;')
	{
	 ...
	 * @return unknown_type
	 */
	public static function renderRecordSetAsTable($title, array $rows)
	{
		$strLens = array();
		...
		$finalStr = '<ul class="move-right" style="width:' . mb_strlen($topBottomLine) . 'em;font-family:courier new,courier,monospace;"><h4>' . (string) $title . "</h4>\n";
		// +-------------+-------------+--------------+
	...
	public function getTableFieldsList($tableName, $skipAutoIncr = false)
	{
		...
		if (false === $r = $this->describeTable($tableName)) {
			throw new Exception('there are no fileds in table');
		}
	...
	public function getDatabaseName()
	{
		if (false === $res = $this->getResultSet("SELECT database() AS db")) {
			throw new Exception('There was a ne error while retrieving the database name');
		}
		list($k, $v) = each($res);
		return $v['db'];
	}
	...
	public function describeDatabase()
	{
		...
		$rows = $this->getResultSet('SHOW TABLES', array(), PDO::FETCH_NUM);
	 ...
	 * @return unknown_type
	 */
	public function deleteTables($tables = null)
	{
	    if (null === $tables) {
		...
		//disable integrity check to avoid : SQLSTATE[23000]: Integrity constraint violation: 1217 Cannot delete or update a parent row: a foreign key constraint fails
		$this->insertUpdateData('SET FOREIGN_KEY_CHECKS=0');
		...
		//reenable integrity checks
		$this->insertUpdateData('SET FOREIGN_KEY_CHECKS=1');
		...
		if (!empty($failedDeletes)) {
			throw new Exception('The tables: ' . implode(', ', $failedDeletes) . ' were not deleted successfully');
		}
	}
	
	/**
	 * Return the tables that match the pattern
	 * 
	 * @param string $regexStr
	 */
	public function getTablesFromRegex($regexStr, $negate = false)
	{
	    return array_filter($this->getTablesList(), function ($tableName) use ($regexStr, $negate) {
	        return (boolean) preg_match($regexStr, $tableName) && !$negate;
	    });
	}
	...
	public function emptyTables($tables = array())
	{
		...
		foreach ($tables as $table) {
			if (!$this->tableExists($table)) {
				throw new Exception('There is no table with name : ' . $tableName . ', in db :' . $this->getDatabaseName());
			}
	...
	public function changeTableScheme($tableName, $pathToTableDefinitionFile)
	{
		...
		$r = new Regex($defStr, new Regex_String_Generic('CREATE TABLE `' . $tableName . '`[^;]+;'));
		if (!$r->match()) {
			throw new Exception('There is no scheme definition for table with name : ' . $tableName . ', in file : ' . $pathToTableDefinitionFile);
		}
	...
	public function getSubset($set1, $set2)
	{
		...
		if (empty($a)) {
			throw new Exception('There is no intersection between the two sets, source : ' . print_r($set1, true) . ' destination : ' . print_r($set2, true));
		}
		return $a;
	}
