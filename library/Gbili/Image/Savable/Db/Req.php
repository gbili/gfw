<?php
namespace Gbili\Image\Savable\Db;
...
extends AbstractReq
{
	 ...
	 * @return unknown_type
	 */
	public function save(SavableImage $i)
	{
		...
		if (false === $idOrFalse) {
			...
			Out::l1("Image with local url : {$i->getFileName()} already exists in Db with id = $idOrFalse, so it was not fileSystemSave()d");
		}
		$i->setId($idOrFalse);
	}
	 ...
	 * abstract a shit
	 * 
	 * @param SavableImage $i
	 * @return unknown_type
	 */
	public function fetch(SavableImage $i)
	{
	 ...
	 * @return unknown_type
	 */
	public function delete(SavableImage $i)
	{
		if ($this->fetch($i)) {
