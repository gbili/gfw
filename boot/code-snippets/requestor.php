 ...
 */
	...
	/*public*/ function saveAction(array $actionData)
	{
		...
		if (!isset($actionData['bluePrintId'])) {
			throw new Engine_BluePrint_Creator_Db_Exception('You must provide a key : bPId, with the bluePrint id to be able to save the action given : ' . print_r($actionData,true));
		}
		
		if (isset($actionData['parentActionId'])) {
			if (!$this->existsAction((integer) $actionData['parentActionId'])) {
				throw new Engine_BluePrint_Creator_Db_Exception('The parentActionId is does not exist given : ' . print_r($actionData['parentActionId'], true));
			}
			...
			if ($this->existsAnyActionForBluePrint($actionData['bluePrintId'])) {
				throw new Engine_BluePrint_Creator_Db_Exception('No parentActionId given, means this is intended to be the root action, prblem is : bluePrint has already a root action, to solve this, pass a parentActionId');
			}
			//this action is intended to be the root action, check if ok
			//only Engine_BluePrint::ACTION_TYPE_GETCONTENTS can be root actions
			if (!isset($actionData['type'])) {
				throw new Engine_BluePrint_Creator_Db_Exception('You must provide the key \'type\', given : ' . print_r($actionData, true));
			}
			if ($actionData['type'] !== Engine_BluePrint::ACTION_TYPE_GETCONTENTS) {
				throw new Engine_BluePrint_Creator_Db_Exception('Only actions of type Engine_BluePrint::ACTION_TYPE_GETCONTENTS can be root');
			}
			//everitying ok
			$isRootAction = true;
		...
		if (!empty($missingEntries)) {
			throw new Engine_BluePrint_Creator_Db_Exception('you have not provided all the data missing keys : ' . print_r($missingEntries, true));
		}
		...
		if (!is_numeric($actionId)) {
			throw new Engine_BluePrint_Creator_Db_Exception('lastInsertId() did not return a numeric');
		}
		...
		//3. particular insert only if 'type' === Engine_BluePrint::ACTION_TYPE_EXTRACT
		if ($actionData['type'] === Engine_BluePrint::ACTION_TYPE_EXTRACT) {
			if (!isset($actionData['data'])) {
				throw new Engine_BluePrint_Creator_Db_Exception('When the action is of type Engine_BluePrint::ACTION_TYPE_EXTRACT, you must pass the key \'data\' along with the $actionData, given: ' . print_r($actionData, true));
			}
